...
public abstract class JRVBaseAdapter<T> extends RecyclerView.Adapter<JRVBaseAdapter.CommomViewHolder> {
    protected List<T> mBeans;
    protected Context mContext;
    protected boolean mAnimateItems = false;
    protected int mLastAnimatedPosition = -1;

    public JRVBaseAdapter(List<T> beans, Context context) {
        mBeans = beans;
        mContext = context;
    }
    ...
    //创建viewholder
    @Override
    public JRVBaseAdapter.CommomViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        LayoutInflater inflater = LayoutInflater.from(mContext);
        View view = inflater.inflate(getItemLayoutID(viewType), parent, false);
        CommomViewHolder holder = new CommomViewHolder(view);
        return holder;
    }

    //绑定数据到视图
    @Override
    public void onBindViewHolder(JRVBaseAdapter.CommomViewHolder holder, int position) {
     ...
     * @param bean
     */
    protected abstract void onBindData(CommomViewHolder holder, T bean);
